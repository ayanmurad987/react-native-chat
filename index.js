...
export default class TwilioChatReactNative extends Component {
  state = {
  ...
  login(username, host) {
    ...
    if (Platform.OS === "ios") {
      chatClientHelper.login(
        username,
        "apns",
        ApnSupport.registerForPushCallback,
        ApnSupport.showPushCallback
      );
  ...
  render() {
    if (this.state.chatClientHelper === null) {
      return <Login host={tokenHost} login={this.login.bind(this)} />;
    } else {
      return <EventsLog eventslog={this.state.log} />;
    }
  }
}

AppRegistry.registerComponent(
  "TwilioChatReactNative",
  () => TwilioChatReactNative
);
